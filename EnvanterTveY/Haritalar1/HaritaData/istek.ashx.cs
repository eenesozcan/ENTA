...
namespace bim.Haritalar.HaritaData
{
    ...
    {
        SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["aykome"].ConnectionString);
        ...
        public string kmz(string id,string filtre)
        {
            ...
            sql = "SELECT DOSYALAR.ID2 FROM RUHSATLAR INNER JOIN DOSYALAR ON DOSYALAR.ID=RUHSATLAR.KMZ WHERE RUHSATLAR.KMZ IS NOT NULL "+filtre;
